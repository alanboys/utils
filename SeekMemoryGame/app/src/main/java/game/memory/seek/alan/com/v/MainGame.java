...
//        playSound(1, 1); // move sound//每次移动的声音
        aGrid.cancelAnimations();
    ...
    private void initSoundPool() {//将声音文件按顺序添加到map
        soudPool = new SoundPool(5, AudioManager.STREAM_MUSIC, 0);
        spMap = new HashMap<Integer, Integer>();
        spMap.put(1, soudPool.load(mView.getContext(), R.raw.sfx_swooshing, 1)); // slide
        spMap.put(2, soudPool.load(mView.getContext(), R.raw.sfx_swooshing, 1)); // get
        // point
        spMap.put(3, soudPool.load(mView.getContext(), R.raw.wing, 1)); // swoosh
        spMap.put(4, soudPool.load(mView.getContext(), R.raw.die, 1)); // die
        spMap.put(5, soudPool.load(mView.getContext(), R.raw.sfx_wing, 1)); // 胜利
    }

    private void playSound(int sound, int number) {//添加声音
